(function () {
  ...
  osm.getSource().on('tileloaderror', () => {
    say('⚠️ OSM tile failed — check Netlify proxy for OSM', 'warn');
  });

  // Rails (OpenRailwayMap, proxied via Netlify)
  ...
  rails.getSource().on('tileloaderror', () => {
    say('⚠️ ORM tile failed — check Netlify proxy for ORM', 'warn');
  });
