(function () {
  ...
  osm.getSource().on('tileloaderror', () => {
    say('⚠️ OSM tile failed — check /tiles/osm proxy', 'warn');
  });
  ...
  rails.getSource().on('tileloaderror', () => {
    say('⚠️ ORM tile failed — check /tiles/orm proxy', 'warn');
  });

  // Carrier overlays (dummy geojsons for now)
  ...
  const refresh = () => {
    ...
    zi.textContent = `z=${(z ?? '–').toFixed?.(2) ?? z ?? '–'} · DPR=${Math.round(devicePixelRatio || 1)}`;
  };
  view.on('change:resolution', refresh); refresh();

  // Logout
  $('logout').onclick = () => {
    try { document.cookie = 'railops_session=; Max-Age=0; path=/; SameSite=Lax'; } catch { }
