// trains.js — Train Tracker 2.0 frontend loader (OpenLayers) — debug HUD + robust fetch
const TrainLayer = (() => {
  // ✅ POINT THIS AT THE REAL JSON YOU’RE PUBLISHING
  const DATA_URL = "https://brilliant-crepe-a70c64.netlify.app/trains.json";

  const REFRESH_MS = 30_000;
  const MAX_AGE_MIN = 120;
  const ENABLE_POPUPS = true;
  const SHOW_HUD = true;               // show top-right status box
  const DEMO_IF_EMPTY = false;         // set true to render 2 demo trains if feed is empty
  ...
  function ensureHud(){
    ...
    Object.assign(hud.style, {
      ...
      maxWidth:"40vw", whiteSpace:"nowrap"
    });
  ...
  async function refresh(){
    ...
    try{
      ...
      let msgBase = `HTTP ${http}`;
      if(!res.ok){
        setHud(`Live trains: ${msgBase} — fetch failed`);
        console.error("[TrainLayer] fetch error", res.status, res.statusText);
        return;
      }
      // Try parse JSON; if it fails, show brief preview of text
      ...
      try {
        ...
        setHud(`Live trains: ${msgBase} — not JSON (${preview || "empty"})`);
        console.error("[TrainLayer] parse error: response is not valid JSON. Preview:", preview);
        return;
      }
