// trains.js — Train Tracker 2.0 frontend loader (OpenLayers)
// UTF-8, no BOM

const TrainLayer = (() => {
  // 👉 point to your Netlify JSON
  const DATA_URL = "https://traintracker2-0.netlify.app/trains.json";

  // refresh window: 30–90s (jittered), helps avoid looking botty
  const MIN_MS = 30_000, MAX_MS = 90_000;
  const MAX_AGE_MIN = 120;         // grey out older than this
  ...
  const upd     = (r) => {
    const c = r.timestamp||r.Timestamp||r.updated||r.Updated||r.LastSeen||r.lastSeen;
  ...
  function norm(r,i){
    ...
    if(r && r.type==="Feature" && r.geometry?.type==="Point"){
      ...
      return { id:id(p,i), lon:pf(lo), lat:pf(la), heading:heading(p), speed:speed(p),
               label:label(p), operator:operator(p), updatedAt:upd(p), raw:r };
    }
    // flat record
    return { id:id(r,i), lon:lon(r), lat:lat(r), heading:heading(r), speed:speed(r),
             label:label(r), operator:operator(r), updatedAt:upd(r), raw:r };
  }

  function style(f){
    const age = f.get("ageMin");
    const hd  = f.get("heading") ?? 0;
    let col = [0,120,255,0.9];                 // fresh → blue
    if (age != null) {
      if (age > MAX_AGE_MIN) col = [130,130,130,0.7];  // stale → grey
      else if (age > 30)     col = [255,165,0,0.9];    // >30m → orange
    }
  ...
  function ensurePopup(){
    ...
    Object.assign(popupEl.style, {
      ...
      border:"1px solid rgba(0,0,0,0.1)", minWidth:"180px"
    });
    popupOverlay = new ol.Overlay({
      element: popupEl, autoPan:{ animation:{ duration:250 } },
      offset:[0,-12], positioning:"bottom-center"
    });
  ...
  function upsert(n){
    ...
    const coord = to3857(n.lon, n.lat);
    const age   = minsAgo(n.updatedAt);
    ...
    f.setProperties({
      ...
      heading:n.heading??0, updatedAt:n.updatedAt??null, lon:n.lon, lat:n.lat,
      ageMin:age??null
    }, true);
    f.setStyle(style(f));
  }
  ...
  async function refresh(){
    try{
      ...
      const arr = extract(raw).map(norm).filter(r => r.lat!=null && r.lon!=null);
      const seen = new Set();
      for(const n of arr){ upsert(n); seen.add(String(n.id)); }
      // restyle (age shading)
      ...
      const wait = Math.floor(MIN_MS + Math.random()*(MAX_MS - MIN_MS));
  ...
  return {
    init(olMap){
      ...
      map.addLayer(layer);
      ensurePopup();
      refresh(); // kicks off the jittered loop
