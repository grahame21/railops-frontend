...
const TrainLayer = (() => {
  ...
  const DEMO_IF_EMPTY = false;   // set true to show 2 demo trains if feed empty
  ...
  function ensureHud(){
    ...
    Object.assign(hud.style, {
      ...
      maxWidth:"42vw", whiteSpace:"nowrap"
    });
  ...
  // --- flexible extractors (handles many JSON shapes)
  function extract(d){
    if(!d) return [];
  ...
  async function refresh(){
    ...
    try{
      ...
      try { raw = JSON.parse(rawText); }
        ...
        console.error("[TrainLayer] parse error: preview:", preview);
        return;
      }
      ...
      setHud(`Live trains: ${count} • updated ${new Date().toLocaleTimeString()} • ${ms}ms`);
